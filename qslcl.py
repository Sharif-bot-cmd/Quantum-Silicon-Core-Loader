#!/usr/bin/env python3
# qslcl.py — Universal QSLCL Tool v1.0.3
...
def load_qslclidx(blob):
    """
    Parse QSLCLIDX table from qslcl.bin.
    Each IDX entry usually contains:
       - 1 byte  index ID
       - 1 byte  flags
       - 2 bytes offset or pointer
       - variable metadata (loader dependent)
    """
    global QSLCLIDX_DB
    QSLCLIDX_DB.clear()
    ...
    while True:
        idx = blob.find(b"QSLCLIDX", off)
        if idx < 0:
            break

        # Size = next marker or EOF
        next_idx = blob.find(b"QSLCL", idx + 8)  # any QSLCL* marker
        if next_idx < 0:
            next_idx = len(blob)

        block = blob[idx:next_idx]
        ...
        # Format: [MAGIC 8 bytes][count 2 bytes][reserved]
        count = int.from_bytes(block[8:10], "little")
        ptr = 16  # first entry after header
        ...
        for i in range(count):
            ...
            entry_id = block[ptr]
            flags = block[ptr+1]
            offset = int.from_bytes(block[ptr+2:ptr+4], "little")

            QSLCLIDX_DB[entry_id] = {
                "id": entry_id,
                "flags": flags,
                "offset": offset,
                "raw": block[ptr:ptr+16]  # store raw entry
            }
    ...
    print(f"[+] QSLCLIDX entries loaded: {found}")
...
def cmd_hello(args=None):
    ...
    print(f"   ENG entries : {len(QSLCLENG_DB)}")
    print(f"   PAR entries : {len(QSLCLPAR_DB)}")
    print(f"   VM5 entries : {len(QSLCLVM5_DB)}")
    print(f"   DISP entries: {len(QSLCLDISP_DB)}")
    print(f"   RTF entries : {len(QSLCLRTF_DB)}")
    print(f"   IDX entries : {len(QSLCLIDX_DB)}")
...
def qslclidx_or_dispatch(dev, cmd_name, payload=b""):
    # IDX ID convention in your loader:
    #  READ      → 0x20
    #  WRITE     → 0x21
    #  ERASE     → 0x22
    #  PEEK      → 0x23
    ...
    idx_id = IDX_MAP.get(cmd_name)
    if idx_id is not None:
        entry = qslclidx_get(idx_id)
        if entry:
            print(f"[*] QSLCLIDX: Using IDX[{idx_id:02X}] for {cmd_name}")
            pkt = b"QSLCLIDX" + entry["raw"] + payload
            return qslcl_dispatch(dev, "IDX", pkt), "IDX"
...
def cmd_read(args):
    ...
    print(f"[*] READ: 0x{aligned_addr:08X} ({aligned_size} bytes)")

    payload = struct.pack("<Q I", aligned_addr, aligned_size)
    resp, origin = qslclidx_or_dispatch(dev, "READ", payload)

    result = _decode_and_show(resp, "READ", aligned_addr, aligned_size, origin)
    if not result:
        return

    data = result["extra"]
    if data:
        print(f"[*] DATA ({len(data)} bytes): {data[:64].hex()}...")
    else:
...
def cmd_write(args):
    ...
    print(f"[*] WRITE: 0x{aligned_addr:08X} ({aligned_len} bytes)")

    payload = struct.pack("<Q", aligned_addr) + data
    resp, origin = qslclidx_or_dispatch(dev, "WRITE", payload)

    _decode_and_show(resp, "WRITE", aligned_addr, aligned_len, origin)
...
def cmd_erase(args):
    ...
    print(f"[*] ERASE: 0x{aligned_addr:08X} ({aligned_size} bytes)")

    payload = struct.pack("<Q I", aligned_addr, aligned_size)
    resp, origin = qslclidx_or_dispatch(dev, "ERASE", payload)

    _decode_and_show(resp, "ERASE", aligned_addr, aligned_size, origin)
...
def main():
    ...
    for cmd_name, cmd_func in [
        ...
        ("lock", cmd_lock), ("reboot", cmd_reboot),
        ("open", cmd_open), ("close", cmd_close), ("test", cmd_test),
        ("checksums", cmd_checksums), ("footer", cmd_footer),
...
# ============================================================
#   AUTO-AUTHENTICATION (only when --auth is provided)
# ============================================================
    if getattr(args, "auth", False):
        ...
        if devs:
            ...
            print("[*] AUTH flag detected — authenticating loader…")

            # extract certificate block from QSLCLHDR_DB
            cert = None
            if "QSLCCERT" in QSLCLHDR_DB:
                cert = QSLCLHDR_DB["QSLCCERT"]
                print(f"[*] QSLCCERT found → {len(cert)} bytes")
            else:
                print("[!] Loader has no QSLCCERT marker. Cannot authenticate.")
                return

            # build payload
            payload = b"QSLCCERT" + cert + b"\x00\x00"

            # dispatch to ENGINE / PAR / VM / fallback
            if 0xA5 in QSLCLENG_DB:
                ...
                v = QSLCLVM5_DB["AUTHENTICATE"]["raw"]
                pkt = b"QSLCLVM5" + v + payload
                resp = qslcl_dispatch(dev, "NANO", pkt)
            else:
                resp = qslcl_dispatch(dev, "AUTHENTICATE", payload)

            # decode response via runtime handler table
            ...
            print("[!] No device for authentication.")
            return
# -------------------------------------------------------

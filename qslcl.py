...
class QSLCLLoader:
    ...
    def load_qslclend(self, blob):
        """Parse QSLCLENG blocks - compatible with both END and ENG naming"""
        out = {}
        # Try both magic values
        for magic in [b"QSLCLEND", b"QSLCLENG"]:
            off = blob.find(magic)
            if off >= 0:
                try:
                    # Parse header: magic(8) + version(1) + flags(1) + count(2)
                    hdr = blob[off:off+12]
                    ...
                    print(f"[!] QSLCLEND/ENG parse error: {e}")
                    continue
    ...
    def load_qslclpar(self, blob):
        ...
        try:
            ...
            hdr = blob[off:off+40]
            magic, version, reserved, name_field, cmd_id, flags, tier, family_hash, length, crc, timestamp = \
                struct.unpack("<8sB3s16sBBBBHII", hdr)
            
            name = name_field.decode("ascii", errors="ignore").rstrip('\x00')
            
            # Extract the command code
            if off + 40 + length <= len(blob):
                raw = blob[off+40:off+40+length]
                out[name] = {
                    "name": name,
                    "cmd_id": cmd_id,
                    "raw": raw,
            ...
            print(f"[!] QSLCLPAR parse error: {e}")
            return out
    ...
    def load_qslclrtf(self, blob):
        ...
        try:
            ...
            for i in range(count):
                if pos + 20 > len(blob):
                    break
                # Fixed format: error_code(4) + severity(1) + category(1) + retry_count(2) + msg_hash(4) + short_name(8)
                code, severity, category, retry_count, msg_hash = struct.unpack("<IBBH I", blob[pos:pos+12])
                short_name = blob[pos+12:pos+20].decode("ascii", errors="ignore").rstrip('\x00')
            ...
            print(f"[!] QSLCLRTF parse error: {e}")
            return out
    ...
    def parse_loader(self, blob):
        # Try both END and ENG naming conventions
        self.load_qslclend(blob)  # This now handles both
        ...
        # Debug output to see what was actually found
        print(f"[*] Parser results:")
        print(f"    ENG/END: {len(self.END)} entries")
        print(f"    PAR: {len(self.PAR)} commands") 
        print(f"    RTF: {len(self.RTF)} entries")
        print(f"    HDR: {len(self.HDR)} blocks")
        
        return len(self.PAR) > 0 or len(self.END) > 0  # More lenient requirement

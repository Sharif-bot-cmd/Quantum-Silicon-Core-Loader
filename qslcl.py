#!/usr/bin/env python3
# qslcl.py — Universal QSLCL Tool v1.0.6
...
class QSLCLLoader:
    def __init__(self):
        ...
        self.HDR  = {}
        self.RTF  = {}
        self.ENG  = {}  # Added for compatibility
    ...
    def parse_loader(self, blob):
        """Master parser with auto-marker scanning and filler/alignment tolerance"""
        print(f"[*] Parsing loader structures ({len(blob)} bytes)...")

        # Known module markers
        MARKERS = [
            b"QSLCLEND", b"QSLCLPAR", b"QSLCLRTF",
            b"QSLCLUSB", b"QSLCLSPT", b"QSLCLVM5",
            b"QSLCLDISP", b"QSLCLIDX", b"QSLCLHDR"
        ]

        # ---- FIX #1: global marker discovery (FULL SCAN) ----
        discovered = {}  # marker -> [offsets]

        i = 0
        blob_len = len(blob)

        while i < blob_len - 8:
            # Skip alignment or filler (0x00 / 0xFF / 0xAA)
            b0 = blob[i]
            if b0 in (0x00, 0xFF, 0xAA):
                i += 1
                continue
            ...
            if chunk in MARKERS:
                ...
                if name not in discovered:
                    discovered[name] = []
                discovered[name].append(i)
                i += 8
                continue

            i += 1

        # ---- FIX #2: resolve payload ranges with "scan-to-next-marker" logic ----
        resolved_blocks = {}   # name -> [payloads]

        for name, offsets in discovered.items():
            resolved_blocks[name] = []
            for start in offsets:
                ...
                next_pos = None
                for m in MARKERS:
                    pos = blob.find(m, start + 8)
                    if pos != -1:
                        if next_pos is None or pos < next_pos:
                            next_pos = pos

                # last block goes to EOF
                if next_pos is None:
                    payload = blob[start + 8:]
                else:
                    payload = blob[start + 8:next_pos]

                # save cleaned block
                resolved_blocks[name].append(payload)

        # ---- FIX #3: feed real payloads to your module parsers ----
        parsers = {
        ...
        for name, parse_fn in parsers.items():
            if name in resolved_blocks:
                for payload in resolved_blocks[name]:
                    try:
                        res = parse_fn(payload)
                        if res:
                            print(f"[*] {name}: {len(res)} entries")
                    except Exception:
                        pass  # silence individual parser errors

        # ---- SUMMARY ----
        print("[*] Parser summary:")
        modules_found = []

        if len(self.END) > 0: modules_found.append(f"END({len(self.END)})")
        if len(self.PAR) > 0: modules_found.append(f"PAR({len(self.PAR)})")
        if len(self.RTF) > 0: modules_found.append(f"RTF({len(self.RTF)})")
        if len(self.HDR) > 0: modules_found.append(f"HDR({len(self.HDR)})")
        if len(self.IDX) > 0: modules_found.append(f"IDX({len(self.IDX)})")
        if len(self.VM5) > 0: modules_found.append(f"VM5({len(self.VM5)})")
        if len(self.USB) > 0: modules_found.append(f"USB({len(self.USB)})")
        if len(self.SPT) > 0: modules_found.append(f"SPT({len(self.SPT)})")
        if len(self.DISP) > 0: modules_found.append(f"DISP({len(self.DISP)})")

        if modules_found:
            print(f"[+] Modules: {', '.join(modules_found)}")
        else:
            print("[!] No valid modules found")

        return len(modules_found) > 0

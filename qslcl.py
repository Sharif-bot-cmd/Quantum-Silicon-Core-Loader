#!/usr/bin/env python3
# qslcl.py — Universal QSLCL Tool v1.2.4
# Author: Sharif — QSLCL Creator
# Works on all SOC
import sys, time, argparse, zlib, struct, threading, re, os, random, math, shutil, gzip, json, itertools, hashlib
...
QSLCLHDR_DB = {}
QSLCLPAR_DB = {}  # Consolidated: contains both name and opcode lookups
QSLCLVM5_DB  = {}
QSLCLUSB_DB  = {}
QSLCLSPT_DB  = {}
QSLCLDISP_DB = {}
...
# =============================================================================
# QSLCL STRUCTURE PARSING UTILITIES (ENHANCED)
# =============================================================================
...
class QSLCLLoader:
    def __init__(self):
        self.PAR  = {}  # Consolidated: contains all commands with opcode lookup
        self.IDX  = {}
        self.VM5  = {}
        self.USB  = {}
        self.SPT  = {}
        ...
        self.HDR  = {}
        self.RTF  = {}
        self.ENG  = {}  # Alias for PAR for compatibility
    ...
    def load_qslclbst(self, blob):
        ...
        struct_info = parse_qslcl_structure(blob, b"QSLCLBST")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLBST: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLBST: CRC mismatch!")
        ...
        while pos < len(blob):
            ...
            try:
                # Parse QSLCLBST header: <8sBBHIII16s
                if idx + 32 > len(blob):
                    pos = idx + 1
                    continue
                ...
                # Calculate offsets for bootstrap components
                bootstrap_start = idx + 32
                
                # Check for security envelope (if secure mode flag is set)
                if flags & 0x01:  # Secure mode
                    security_start = bootstrap_start
                    if security_start + 24 <= len(blob):
                        # Parse security header: <II16s
                        security_magic, security_crc, security_hash = \
                            struct.unpack("<II16s", blob[security_start:security_start+24])
                        bootstrap_start += 24
                
                # Extract bootstrap code
                if bootstrap_start + code_size <= len(blob):
                    bootstrap_code = blob[bootstrap_start:bootstrap_start + code_size]
                    ...
                    if data_start + 256 <= len(blob):
                        bootstrap_data = blob[data_start:data_start + 256]
                    else:
                        bootstrap_data = b""
                    
                    # Extract footer
                    footer_start = data_start + 256
                    if footer_start + 32 <= len(blob):
                        footer_magic, footer_crc, footer_hash, footer_id = \
                            struct.unpack("<II16s8s", blob[footer_start:footer_start+32])
                    else:
    ...
    # ---------------------------------------------
    # ENHANCED QSLCLPAR PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslclpar(self, blob):
        """Enhanced QSLCLPAR parser with proper structured format handling"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLPAR")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLPAR: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLPAR: CRC mismatch!")
        
        # Search for QSLCLPAR magic in body
        pos = 0
        while pos < len(blob):
            ...
            try:
                # Check if this is a structured header
                if idx + 20 <= len(blob):
                    # Try to parse as structured format first
                    magic, body_size, flags, crc = struct.unpack("<8sIII", blob[idx:idx+20])
                    
                    if magic == b"QSLCLPAR":
                        # This is a structured format header
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLPAR: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        
                        # Extract and verify body
                        body_data = blob[body_start:body_end]
                        calculated_crc = zlib.crc32(body_data) & 0xFFFFFFFF
                        crc_valid = (crc == calculated_crc)
                        
                        if not crc_valid:
                            print(f"[!] QSLCLPAR: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLPAR: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual command data from the body
                        cmd_blob = body_data
                        cmd_idx = 0
                        
                        # Check for command header within body
                        if len(cmd_blob) >= 16:
                            # Parse the actual command data header
                            cmd_magic, version, cmd_flags, count, data_len = struct.unpack("<8sBBHI", cmd_blob[cmd_idx:cmd_idx+16])
                            
                            if cmd_magic != b"QSLCLPAR":
                                print(f"[!] QSLCLPAR: Invalid command magic: {cmd_magic}")
                                pos = idx + 1
                                continue
                            
                            print(f"[+] QSLCLPAR: Command data header (v{version}, {count} commands, data_len={data_len})")
                            
                            cmd_pos = cmd_idx + 16
                            commands_found = 0
                            
                            for i in range(count):
                                if cmd_pos + 40 > len(cmd_blob):
                                    break
                                    
                                # Parse command header: <16sBBBBHII
                                cmd_hdr = cmd_blob[cmd_pos:cmd_pos+40]
                                name_field, opcode, cmd_flags, tier, family_hash, length, cmd_crc, timestamp = \
                                ...
                                if (not name or len(name) < 2 or not name.isalnum() or
                                    cmd_pos + 40 + length > len(cmd_blob) or length > 4096):
                                    cmd_pos += 1
                                    continue
                                    
                                # Extract command data
                                cmd_data = cmd_blob[cmd_pos+40:cmd_pos+40+length]
                                ...
                                # Store by name and by opcode for compatibility
                            ...
                            if commands_found > 0:
                                print(f"[*] QSLCLPAR: Found {commands_found} commands in structured format (v{version})")
                                break
                        ...
                        hdr = blob[idx:idx+16]
                        magic, version, flags, count, data_len = struct.unpack("<8sBBHI", hdr)
                        ...
                        for i in range(count):
                            ...
                            # Validate name and check data bounds
                            if (not name or len(name) < 2 or not name.isalnum() or
                                cmd_pos + 40 + length > len(blob) or length > 4096):
                                cmd_pos += 1
                                continue
                        ...
                        if commands_found > 0:
                            print(f"[*] QSLCLPAR: Found {commands_found} commands in legacy format (v{version})")
                            break
                            
            except Exception as e:
                print(f"[!] QSLCLPAR parse error at 0x{idx:X}: {e}")
                traceback.print_exc()
                pass
    ...
    # ---------------------------------------------
    # ENHANCED QSLCLDISP PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslcldisp(self, blob):
        """Enhanced QSLCLDISP parser with structured format support"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLDIS")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLDISP: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLDISP: CRC mismatch!")
        
        # Search for QSLCLDIS magic
        magic = b"QSLCLDIS"
        pos = 0
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if idx + 20 <= len(blob):
                    ...
                    magic_found, body_size, flags, crc = struct.unpack("<8sIII", blob[idx:idx+20])
                    
                    if magic_found == b"QSLCLDIS":
                        # This is a structured format
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLDISP: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        ...
                        if not crc_valid:
                            print(f"[!] QSLCLDISP: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLDISP: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual dispatch data from the body
                        disp_blob = body_data
                        disp_idx = 0
                        
                        # Check for dispatch header within body
                        if len(disp_blob) >= 16:
                            # Parse the actual dispatch header: <8sHHI
                            disp_magic, version, disp_flags, count = struct.unpack("<8sHHI", disp_blob[disp_idx:disp_idx+16])
                            
                            if disp_magic != b"QSLCLDIS":
                                print(f"[!] QSLCLDISP: Invalid dispatch magic in body: {disp_magic}")
                                pos = idx + 1
                                continue
                            
                            print(f"[+] QSLCLDISP: Dispatch data header (v{version}, {count} entries)")
                            
                            entry_pos = disp_idx + 16
                            entries_found = 0
                            
                            for i in range(count):
                                if entry_pos + 12 > len(disp_blob):
                                    break
                                    
                                # Entry format: <8sI (cmd_hash, handler_addr)
                                cmd_hash, handler_addr = struct.unpack("<8sI", disp_blob[entry_pos:entry_pos+12])
                            ...
                            if entries_found > 0:
                                print(f"[*] QSLCLDISP: Found {entries_found} dispatch entries in structured format")
                                break
                        ...
                        if entries_found > 0:
                            print(f"[*] QSLCLDISP: Found {entries_found} dispatch entries in legacy format")
                            break
    ...
    # ---------------------------------------------
    # ENHANCED RTF PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslclrtf(self, blob):
        """Enhanced RTF parser with structured format support"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLRTF")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLRTF: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLRTF: CRC mismatch!")
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if idx + 20 <= len(blob):
                    ...
                    if magic_found == b"QSLCLRTF":
                        # This is a structured format
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLRTF: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        ...
                        if not crc_valid:
                            print(f"[!] QSLCLRTF: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLRTF: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual RTF data from the body
                        rtf_blob = body_data
                        rtf_idx = 0
                        
                        # Check for RTF header within body
                        if len(rtf_blob) >= 12:
                            # Parse the actual RTF header: <8sBBH
                            rtf_magic, ver, rtf_flags, count = struct.unpack("<8sBBH", rtf_blob[rtf_idx:rtf_idx+12])
                            
                            if rtf_magic != b"QSLCLRTF":
                                print(f"[!] QSLCLRTF: Invalid RTF magic in body: {rtf_magic}")
                                pos = idx + 1
                                continue
                            
                            print(f"[+] QSLCLRTF: RTF data header (v{ver}, {count} entries)")
                            
                            entry_pos = rtf_idx + 12
                            entries_found = 0
                            
                            for i in range(count):
                                if entry_pos + 12 > len(rtf_blob):
                                    break
                                
                                # Fixed format: error_code(4) + severity(1) + category(1) + retry_count(2) + msg_hash(4)
                                code, severity, category, retry_count, msg_hash = struct.unpack("<IBBH I", rtf_blob[entry_pos:entry_pos+12])
                                
                                # Extract short name (8 bytes)
                                name_end = entry_pos + 20
                                if name_end > len(rtf_blob):
                                    break
                                short_name = rtf_blob[entry_pos+12:name_end].decode("ascii", errors="ignore").rstrip('\x00')
                            ...
                            if entries_found > 0:
                                print(f"[*] QSLCLRTF: Found {entries_found} entries in structured format")
                                break
                        ...
                        if entries_found > 0:
                            print(f"[*] QSLCLRTF: Found {entries_found} entries in legacy format")
                            break
    ...
    # ---------------------------------------------
    # ENHANCED IDX PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslclidx(self, blob):
        """Enhanced IDX parser with structured format support"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLIDX")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLIDX: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLIDX: CRC mismatch!")
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if idx + 20 <= len(blob):
                    ...
                    if magic_found == b"QSLCLIDX":
                        # This is a structured format
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLIDX: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        ...
                        if not crc_valid:
                            print(f"[!] QSLCLIDX: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLIDX: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual IDX data from the body
                        idx_blob = body_data
                        idx_pos = 0
                        
                        # Check for IDX header within body
                        if len(idx_blob) >= 12:
                            # Parse the actual IDX header: <8sBBH
                            idx_magic, ver, idx_flags, count = struct.unpack("<8sBBH", idx_blob[idx_pos:idx_pos+12])
                            
                            if idx_magic != b"QSLCLIDX":
                                print(f"[!] QSLCLIDX: Invalid IDX magic in body: {idx_magic}")
                                pos = idx + 1
                                continue
                            
                            print(f"[+] QSLCLIDX: IDX data header (v{ver}, {count} entries)")
                            
                            entry_pos = idx_pos + 12
                            entries_found = 0
                            
                            for _ in range(count):
                                if entry_pos + 3 > len(idx_blob):
                                    break
                                    
                                idx_val = struct.unpack("<H", idx_blob[entry_pos:entry_pos+2])[0]
                                name_len = idx_blob[entry_pos+2]
                                
                                if entry_pos + 3 + name_len > len(idx_blob) or name_len == 0 or name_len > 64:
                                    break
                                    
                                name = idx_blob[entry_pos+3:entry_pos+3+name_len].decode("ascii", errors="ignore")
                            ...
                            if entries_found > 0:
                                print(f"[*] QSLCLIDX: Found {entries_found} entries in structured format")
                                break
                        ...
                        for _ in range(count):
                            ...
                            # Only add if name is valid
                            if name and name.isprintable():
                                out[name] = {"idx": idx_val, "name": name, "structured": False}
                                entry_pos += 3 + name_len
                                entries_found += 1
                            else:
                                break
                        ...
                        if entries_found > 0:
                            print(f"[*] QSLCLIDX: Found {entries_found} entries in legacy format")
                            break
    ...
    # ---------------------------------------------
    # ENHANCED VM5 PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslclvm5(self, blob):
        """Enhanced VM5 parser with structured format support"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLVM5")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLVM5: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLVM5: CRC mismatch!")
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if idx + 20 <= len(blob):
                    ...
                    if magic_found == b"QSLCLVM5":
                        # This is a structured format
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLVM5: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        ...
                        if not crc_valid:
                            print(f"[!] QSLCLVM5: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLVM5: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual VM5 data from the body
                        vm5_blob = body_data
                        vm5_pos = 0
                        
                        # Check for VM5 header within body
                        if len(vm5_blob) >= 12:
                            # Parse the actual VM5 header: <8sBBH
                            vm5_magic, ver, vm5_flags, count = struct.unpack("<8sBBH", vm5_blob[vm5_pos:vm5_pos+12])
                            
                            if vm5_magic != b"QSLCLVM5":
                                print(f"[!] QSLCLVM5: Invalid VM5 magic in body: {vm5_magic}")
                                pos = idx + 1
                                continue
                            
                            print(f"[+] QSLCLVM5: VM5 data header (v{ver}, {count} entries)")
                            
                            entry_pos = vm5_pos + 12
                            entries_found = 0
                            
                            for _ in range(count):
                                if entry_pos + 1 > len(vm5_blob):
                                    break
                                    
                                name_len = vm5_blob[entry_pos]
                                
                                if entry_pos + 1 + name_len + 2 > len(vm5_blob) or name_len == 0 or name_len > 64:
                                    break
                                    
                                name = vm5_blob[entry_pos+1 : entry_pos+1+name_len].decode("ascii", errors="ignore")
                                raw_len = struct.unpack("<H", vm5_blob[entry_pos+1+name_len : entry_pos+3+name_len])[0]
                                
                                if entry_pos + 3 + name_len + raw_len > len(vm5_blob) or raw_len > 4096:
                                    break
                                    
                                raw = vm5_blob[entry_pos+3+name_len : entry_pos+3+name_len+raw_len]
                            ...
                            if entries_found > 0:
                                print(f"[*] QSLCLVM5: Found {entries_found} entries in structured format")
                                break
                        ...
                        for _ in range(count):
                            ...
                            # Only add if name is valid
                            if name and name.isprintable():
                                out[name] = {"name": name, "raw": raw, "structured": False}
                                entry_pos += 3 + name_len + raw_len
                                entries_found += 1
                            else:
                                break
                        ...
                        if entries_found > 0:
                            print(f"[*] QSLCLVM5: Found {entries_found} entries in legacy format")
                            break
    ...
    # ---------------------------------------------
    # ENHANCED USB ROUTINES PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslclusb(self, blob):
        """Enhanced USB parser with structured format support"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLUSB")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLUSB: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLUSB: CRC mismatch!")
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if idx + 20 <= len(blob):
                    ...
                    if magic_found == b"QSLCLUSB":
                        # This is a structured format
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLUSB: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        ...
                        if not crc_valid:
                            print(f"[!] QSLCLUSB: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLUSB: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual USB data from the body
                        usb_blob = body_data
                        usb_pos = 0
                        
                        # Check for USB header within body
                        if len(usb_blob) >= 12:
                            # Parse the actual USB header: <8sBBH
                            usb_magic, ver, usb_flags, count = struct.unpack("<8sBBH", usb_blob[usb_pos:usb_pos+12])
                            
                            if usb_magic != b"QSLCLUSB":
                                print(f"[!] QSLCLUSB: Invalid USB magic in body: {usb_magic}")
                                pos = idx + 1
                                continue
                            
                            print(f"[+] QSLCLUSB: USB data header (v{ver}, {count} entries)")
                            
                            entry_pos = usb_pos + 12
                            entries_found = 0
                            
                            for _ in range(count):
                                if entry_pos + 1 > len(usb_blob):
                                    break
                                    
                                name_len = usb_blob[entry_pos]
                                
                                if entry_pos + 1 + name_len + 2 > len(usb_blob) or name_len == 0 or name_len > 64:
                                    break
                                    
                                name = usb_blob[entry_pos+1 : entry_pos+1+name_len].decode("ascii", errors="ignore")
                                raw_len = struct.unpack("<H", usb_blob[entry_pos+1+name_len : entry_pos+3+name_len])[0]
                                
                                if entry_pos + 3 + name_len + raw_len > len(usb_blob) or raw_len > 4096:
                                    break
                                    
                                raw = usb_blob[entry_pos+3+name_len : entry_pos+3+name_len+raw_len]
                            ...
                            if entries_found > 0:
                                print(f"[*] QSLCLUSB: Found {entries_found} entries in structured format")
                                break
                        ...
                        if entries_found > 0:
                            print(f"[*] QSLCLUSB: Found {entries_found} entries in legacy format")
                            break
    ...
    # ---------------------------------------------
    # ENHANCED SPT SETUP PACKETS PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslclspt(self, blob):
        """Enhanced SPT parser with structured format support"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLSPT")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLSPT: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLSPT: CRC mismatch!")
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if idx + 20 <= len(blob):
                    ...
                    if magic_found == b"QSLCLSPT":
                        # This is a structured format
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLSPT: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        ...
                        if not crc_valid:
                            print(f"[!] QSLCLSPT: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLSPT: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual SPT data from the body
                        spt_blob = body_data
                        spt_pos = 0
                        
                        # Check for SPT header within body
                        if len(spt_blob) >= 12:
                            # Parse the actual SPT header: <8sBBH
                            spt_magic, ver, spt_flags, count = struct.unpack("<8sBBH", spt_blob[spt_pos:spt_pos+12])
                            
                            if spt_magic != b"QSLCLSPT":
                                print(f"[!] QSLCLSPT: Invalid SPT magic in body: {spt_magic}")
                                pos = idx + 1
                                continue
                            
                            print(f"[+] QSLCLSPT: SPT data header (v{ver}, {count} entries)")
                            
                            entry_pos = spt_pos + 12
                            entries_found = 0
                            
                            for _ in range(count):
                                if entry_pos + 1 > len(spt_blob):
                                    break
                                    
                                name_len = spt_blob[entry_pos]
                                
                                if entry_pos + 1 + name_len + 2 > len(spt_blob) or name_len == 0 or name_len > 64:
                                    break
                                    
                                name = spt_blob[entry_pos+1 : entry_pos+1+name_len].decode("ascii", errors="ignore")
                                raw_len = struct.unpack("<H", spt_blob[entry_pos+1+name_len : entry_pos+3+name_len])[0]
                                
                                if entry_pos + 3 + name_len + raw_len > len(spt_blob) or raw_len > 4096:
                                    break
                                    
                                raw = spt_blob[entry_pos+3+name_len : entry_pos+3+name_len+raw_len]
                            ...
                            if entries_found > 0:
                                print(f"[*] QSLCLSPT: Found {entries_found} entries in structured format")
                                break
                        ...
                        if entries_found > 0:
                            print(f"[*] QSLCLSPT: Found {entries_found} entries in legacy format")
                            break
    ...
    # ---------------------------------------------
    # ENHANCED HEADER / CERTS PARSER - FIXED FOR STRUCTURED FORMAT
    # ---------------------------------------------
    def load_qslclhdr(self, blob):
        """Enhanced HDR parser with structured format support"""
        out = {}
        
        # First try to parse as structured format
        struct_info = parse_qslcl_structure(blob, b"QSLCLHDR")
        if struct_info:
            blob = struct_info["body"]
            print(f"[+] QSLCLHDR: Structured format detected (size={struct_info['size']}, flags=0x{struct_info['flags']:08X})")
            if not struct_info["crc_valid"]:
                print("[!] QSLCLHDR: CRC mismatch!")
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if idx + 20 <= len(blob):
                    ...
                    if magic_found == b"QSLCLHDR":
                        # This is a structured format
                        body_start = idx + 20
                        body_end = body_start + body_size
                        
                        if body_end > len(blob):
                            print(f"[!] QSLCLHDR: Body size {body_size} exceeds available data")
                            pos = idx + 1
                            continue
                        ...
                        if not crc_valid:
                            print(f"[!] QSLCLHDR: CRC mismatch (stored: 0x{crc:08X}, calculated: 0x{calculated_crc:08X})")
                        
                        print(f"[+] QSLCLHDR: Structured header found (body_size={body_size}, flags=0x{flags:08X})")
                        
                        # Now parse the actual HDR data from the body
                        hdr_blob = body_data
                        hdr_pos = 0
                        
                        # HDR blocks can be nested, so we need to look for the actual header format
                        # Common HDR format: <8sII (magic(8) + version(4) + size(4))
                        if hdr_pos + 16 <= len(hdr_blob):
                            hdr_magic = hdr_blob[hdr_pos:hdr_pos+8]
                            
                            if hdr_magic == b"QSLCLHDR":
                                # Parse HDR data header
                                ver, size = struct.unpack("<II", hdr_blob[hdr_pos+8:hdr_pos+16])
                                
                                data_start = hdr_pos + 16
                                data_end = data_start + size
                                
                                if data_end <= len(hdr_blob):
                                    digest = hdr_blob[data_start:data_start+16] if data_start + 16 <= len(hdr_blob) else b""
                                    payload_start = data_start + 16
                                    payload_end = min(data_end, len(hdr_blob))
                                    
                                    if payload_start < payload_end:
                                        payload = hdr_blob[payload_start:payload_end]
                                        ...
                                        print(f"[*] QSLCLHDR: Found structured block (size={size} bytes, ver={ver})")
                                        pos = idx + 20 + body_size
                                        continue
                        ...
                        print(f"[*] QSLCLHDR: Found legacy block ({size} bytes, ver={ver})")
                        pos = idx + 32 + size
                        continue
    ...
    # ---------------------------------------------
    # ENHANCED MASTER PARSER WITH PROPER STRUCTURED FORMAT SUPPORT
    # ---------------------------------------------
    def parse_loader(self, blob):
        """Parse QSLCL binary with proper structured format support"""
        print(f"[*] Parsing loader ({len(blob)} bytes)...")
        
        # First, check if this is a structured QSLCLBIN
        if len(blob) >= 20 and blob[:8] == b"QSLCLBIN":
            print("[+] Detected structured QSLCL binary format")
            try:
                # Parse main binary header: [QSLCLBIN][size][flags][crc]
                size, flags, stored_crc = struct.unpack("<III", blob[8:20])
                body_start = 20
                body_end = body_start + size
                
                if body_end > len(blob):
                    print(f"[!] Invalid body size: {size} > available {len(blob)-20}")
                ...
                print(f"[!] Failed to parse QSLCL binary header: {e}")
        
        # Extended marker set
        ALL_MARKERS = [
            b"QSLCLBST", b"QSLCLPAR", b"QSLCLRTF", b"QSLCLUSB", b"QSLCLSPT", 
            b"QSLCLVM5", b"QSLCLDISP", b"QSLCLIDX", b"QSLCLHDR", 
            b"QSLCLCMD", b"QSLCLBIN", b"QSLCLPKT", b"QSLCLRESP", 
        ...
        # Parse structures in order of importance
        success_count = 0
        parse_order = ["QSLCLBST", "QSLCLPAR", "QSLCLDISP", "QSLCLRTF", 
        ...
        if self.PAR:  found_modules.append(f"PAR({len(self.PAR)//2})")
        ...
        if found_modules:
            ...
            if self.PAR:
                ...
                command_names = [name for name in self.PAR.keys() if isinstance(name, str) and name.isalpha()]
...
# =============================================================================
# ENHANCED VALIDATION FUNCTION WITH PROPER STRUCTURED FORMAT SUPPORT
# =============================================================================
def validate_binary_compatibility(blob):
    """Validate QSLCL binary with proper structured format support"""
    if len(blob) < 8:
    ...
    if blob[:8] == b"QSLCLBIN":
        if len(blob) < 20:
            print("[!] Structured binary too small")
            return False
        ...
        try:
            ...
            print(f"    Body size: {size} bytes")
            print(f"    Flags: 0x{flags:08X}")
            print(f"    CRC: {'VALID' if calculated_crc == stored_crc else 'INVALID'}")
            
            # Check for required blocks in body
            required_blocks = [b"QSLCLPAR"]
            found_blocks = []
            
            for block in required_blocks:
                if block in body_data:
            ...
            print(f"[!] Structured binary validation error: {e}")
...
# =============================================================================
# CONSOLIDATED COMMAND DISPATCH - UPDATED FOR STRUCTURED FORMAT
# =============================================================================
def qslcl_dispatch(dev, cmd_name, payload=b"", timeout=1.0):
    """
    Unified dispatcher using only QSLCLPAR (consolidated system)
    """
    cmd_upper = cmd_name.upper()

    # Single source: QSLCLPAR (supports both name and opcode lookup)
    if cmd_upper in QSLCLPAR_DB:
        cmd_entry = QSLCLPAR_DB[cmd_upper]
        print(f"[*] QSLCLPAR dispatch → {cmd_upper} (opcode: 0x{cmd_entry['opcode']:02X})")
    ...
    try:
        ...
        if opcode in QSLCLPAR_DB:
            cmd_entry = QSLCLPAR_DB[opcode]
            print(f"[*] QSLCLPAR dispatch → opcode 0x{opcode:02X} ({cmd_entry['name']})")
...
def detect_sector_size(dev):
    ...
    Updated for structured QSLCLPAR system.
    """
    ...
    # ============================================================
    # 1. QSLCLPAR GETSECTOR handler (primary method)
    # ============================================================
...
def send_packets(handle, data, serial_mode, chunk=4096):
    ...
    if total >= 20 and data[:8] == b"QSLCLBIN":
        print("[*] Sending structured QSLCL binary...")
        # Parse header for information
        try:
            size, flags, stored_crc = struct.unpack("<III", data[8:20])
            print(f"[+] Structured binary: size={size}, flags=0x{flags:08X}")
        except:
            pass
    else:
...
def handle_authentication(args):
    ...
    # ---------------------------------------------------------------
    # Priority 1 — QSLCLPAR AUTHENTICATE command
    if "AUTHENTICATE" in QSLCLPAR_DB:
        print("[*] AUTH via QSLCLPAR AUTHENTICATE command")
    ...
    # Priority 3 — opcode A5 (if exists in QSLCLPAR)
    elif 0xA5 in QSLCLPAR_DB:
        print("[*] AUTH via QSLCLPAR opcode A5")
...
def auto_loader_if_needed(args, dev):
    ...
    Updated for structured format support.
    """
    ...
    try:
        ...
        print(f"    QSLCLPAR: {len(loader.PAR)//2} commands")  # Count unique commands
    ...
    # ============================================================
    required = ["QSLCLPAR"]  # Only QSLCLPAR is required now
...
def cmd_hello(args=None):
    ...
    # Use consolidated QSLCLPAR system
    ...
    unique_commands = len([name for name in QSLCLPAR_DB.keys() if isinstance(name, str) and name.isalpha()])
    print(f"  PAR commands : {unique_commands}")

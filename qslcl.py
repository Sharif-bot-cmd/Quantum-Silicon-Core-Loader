#!/usr/bin/env python3
# qslcl.py — Universal QSLCL Tool v1.0.5
...
# =============================================================================
def load_qslclpar(blob_or_path):
...
def auto_loader_if_needed(args, dev):
    ...
    try:
        ...
        print("[!] Loader parsing failure:", e)
        return
    ...
    print(f"    QSLCLENG: {len(QSLCLENG_DB)} entries")
    print(f"    QSLCLPAR: {len(QSLCLPAR_DB)} commands")
    print(f"    QSLCLIDX: {len(QSLCLIDX_DB)} indices")
    print(f"    QSLCLVM5: {len(QSLCLVM5_DB)} microsvcs")
    print(f"    QSLCLUSB: {len(QSLCLUSB_DB)} blocks")
    print(f"    QSLCLSPT: {len(QSLCLSPT_DB)} blocks")
    print(f"    QSLCLHDR: {len(QSLCLHDR_DB)} blocks")
    print()

    # ============================================================
    # 3. Verify loader contains minimum required segments
    # ============================================================
    required = ["QSLCLPAR", "QSLCLENG", "QSLCLRTF"]
...
def load_qslcldisp(blob):
    ...
    print(f"[+] QSLCLDIS dispatcher loaded: {found} entries")
...
def parse_qslclidx_block(blob):
    ...
    Returns mapping: entry_id -> { cmd, offset, size, raw }
    """
    out = {}
    if not blob:
        return out
    ...
    while idx + ENTRY_SZ <= L:
        try:
            ...
            # sanitise offset/size and extract raw slice if possible
            if offset >= 0 and size > 0 and offset + size <= L:

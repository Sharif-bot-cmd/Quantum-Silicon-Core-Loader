    ...
    def load_qslclpar(self, blob):
        ...
        while pos < len(blob):
            ...
            try:
                ...
                if structured:
                    # Parse from structured body
                    if len(struct_body) < 2:
                        pos = idx + 1
                        continue
                    
                    # Parse header from structured body: <BBHI (version, flags, count, data_len)
                    version, flags, count, data_len = struct.unpack("<BBHI", struct_body[:8])
                    cmd_pos = 8
                    commands_found = 0
                else:
                    # Legacy format
                ...
                for i in range(count):
                    if cmd_pos + 40 > len(struct_body):  # Minimum command header size
                        break
                        
                    # Parse command header: <16sBBBBHII
                    cmd_hdr = struct_body[cmd_pos:cmd_pos+40]
                    ...
                    if (not name or len(name) < 2 or not name.isalnum() or
                        cmd_pos + 40 + length > len(struct_body) or length > 4096):
                        cmd_pos += 1
                        continue
                        
                    # Extract command data
                    cmd_data = struct_body[cmd_pos+40:cmd_pos+40+length]

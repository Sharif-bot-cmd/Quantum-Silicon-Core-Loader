#!/usr/bin/env python3
# qslcl.py — Universal QSLCL Tool v1.1.4
...
class QSLCLLoader:
    ...
    # ---------------------------------------------
    # MASTER PARSER - UPDATED FOR CONSOLIDATED SYSTEM
    # ---------------------------------------------
    def parse_loader(self, blob):
        """Updated parser without QSLCLEND"""
        ...
        # Extended marker set (QSLCLEND removed)
        ALL_MARKERS = [
            b"QSLCLPAR", b"QSLCLRTF", b"QSLCLUSB", b"QSLCLSPT", 
...
def validate_binary_compatibility(blob):
    """Check if binary matches parser expectations"""
    if len(blob) < 8:
...
def parse_device_info(resp):
    """Properly parse device information from response"""
    info = {
        "version": "Unknown",

...
# ============================================================
def load_qslclhdr(blob, debug=False):
    """
    Parse QSLCLHDR table and populate QSLCLHDR_DB.
    Table format:
        8s  marker      ("QSLCLHDR")
        I   version
        I   entry_count
    ...
    while off + 16 <= end:
        marker, ver, count = struct.unpack_from("<8sII", blob, off)
        marker = marker.rstrip(b"\x00")

        if marker != b"QSLCLHDR":
            off += 4   # scan forward conservatively
            continue

        off += 16
        if debug:
            print(f"[*] QSLCLHDR found @ 0x{off:08X}, version={ver}, entries={count}")
        ...
        for _ in range(count):
            ...
            name, length = struct.unpack_from("<8sI", blob, off)
            off += 12

            name = name.rstrip(b"\x00").decode("ascii", "ignore")
            if off + length > end:
                break

...
def dynamic_bootstrap(
    ...
    # ============================================================
    # QSLCLBST HEADER (Consistent with other QSLCL components)
    # ============================================================
    MAGIC = b"QSLCLBST"  # 8-byte magic identifier
    VERSION = 0x05       # Version 5.0
    FLAGS = 0x01 if secure_mode else 0x00
    
    # Calculate bootstrap bytecode first to get accurate size
    ...
    # ============================================================
    # QSLCLBST HEADER CONSTRUCTION
    # ============================================================
    
    bootstrap_size = len(universal_bootstrap)
    bootstrap_crc = zlib.crc32(universal_bootstrap) & 0xFFFFFFFF
    timestamp = int(time.time())
    
    # Build QSLCLBST header: <8s B B H I I I 16s
    bootstrap_header = struct.pack(
        "<8sBBHIII16s",
        MAGIC,                          # QSLCLBST magic
        VERSION,                        # Version 5
        FLAGS,                          # Security flags
        len(arch_optimizations),        # Architecture count
        bootstrap_size,                 # Bootstrap code size
        bootstrap_crc,                  # Integrity CRC
    ...
    bootstrap_footer = struct.pack("<II16s8s",
    ...
    if debug:
        ...
        print(f"    Header: QSLCLBST v{VERSION}")
...
def embed_universal_bootstrap(
    ...
    """
    Embed 100% universal bootstrap into qslcl.bin with QSLCLBST header
    ...
    - Includes QSLCLBST header for consistency
    """
    
    # Generate universal bootstrap with QSLCLBST header
    ...
    # Add bootstrap metadata pointer at standard location
    bootstrap_ptr = struct.pack("<I", bootstrap_offset)
    image[0x00:0x04] = bootstrap_ptr  # Store at beginning of image
    
    # Also store bootstrap header info for easy parsing
    bootstrap_info = struct.pack("<II", 
                                len(bootstrap_code), 
                                zlib.crc32(bootstrap_code) & 0xFFFFFFFF)
    image[0x04:0x0C] = bootstrap_info
    
    if debug:
        # Parse and display bootstrap header info
        try:
            magic = image[bootstrap_offset:bootstrap_offset + 8]
            version = image[bootstrap_offset + 8]
            flags = image[bootstrap_offset + 9]
            arch_count = struct.unpack("<H", image[bootstrap_offset + 10:bootstrap_offset + 12])[0]
            code_size = struct.unpack("<I", image[bootstrap_offset + 12:bootstrap_offset + 16])[0]
            
            print(f"[*] QSLCLBST bootstrap embedded @0x{bootstrap_offset:X}")
            ...
            print(f"[*] Bootstrap embedded @0x{bootstrap_offset:X} ({len(bootstrap_code)} bytes)")
...
def build_qslcl_bin(
    ...
    embed_certificate_strings(
        image,
        cert_text=f"QSLCL Universal Binary v5.8\nArchitecture: {arch}\nBuild: {time.ctime()}",
        auth_key=auth_key,
    ...
    if debug:
        print(f"\n[*] QSLCL Universal Binary v5.8 Build Complete")

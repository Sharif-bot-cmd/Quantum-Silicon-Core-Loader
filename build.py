...
def generate_command_code(
    ...
    # ------------------------------------------------------------------
    # 5. Header - STANDARD FORMAT: MAGIC + body_size + flags + crc
    # ------------------------------------------------------------------
    flags = 0x01
    ...
    # Create header with standard format
    header = struct.pack("<8sIII", header_magic, body_size, flags, body_crc)
    
    buf = bytearray(header) + body

    # Secure HMAC header (outer wrapper)
    if secure_mode and include_header:
        # Build outer body (header + inner body)
        outer_body = buf
        outer_body_size = len(outer_body)
        outer_body_crc = zlib.crc32(outer_body) & 0xFFFFFFFF
        
        # Create outer header
        outer_header = bytearray()
        outer_header += b"QSLCLWRP"  # Outer wrapper magic
        outer_header += struct.pack("<III", outer_body_size, flags, outer_body_crc)
...
def build_qslcl_bin(
    ...
    # Create QSLCLPAR command table with standard header
    ...
    endpoint_header = struct.pack("<8sIII", b"QSLCLUSB", endpoint_body_size, endpoint_flags, endpoint_body_crc)
    ...
    if debug:
        ...
        print(f"      - QSLCLPAR: Command implementations @0x{qslclpar_offset:X}")

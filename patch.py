...
def build_img4_trust_capsule(elf: bytes) -> bytes:
    import plistlib, os, struct, hashlib
    try:
        from Crypto.Hash import SHA384
    except ImportError:
        from hashlib import sha384 as SHA384

    # Safely create TLV structure
    ...
    # Dynamically create fake ASN.1 header prefix (entropy-based)
    def generate_asn1_prefix() -> str:
        base = b'\x30' + os.urandom(1)  # ASN.1 SEQ + random length
        version = b'\x02\x01\x01'  # INTEGER version 1
        oid_prefix = b'\x30\x0D\x06\x09\x60\x86' + os.urandom(3)  # Fake OID
        return (base + version + oid_prefix).hex()

    # Apple-style manifest with maximum spoof and undefined compatibility
    ...
    # Assemble full IMG4 block internally
    ...
    # Runtime assembly steps
    im4p = create_im4p(elf)
    try:
        digest = SHA384.new(elf).digest()  # PyCryptodome
    except AttributeError:
        digest = SHA384(elf).digest()      # hashlib fallback
...
def inject_img4_capsule(buffer, capsule, offset: int = 0x3000):
    """
    ♾️ Infinity-Class IMG4 Capsule Injector

    - Obfuscates IMG4 capsule inside ELF memory map
    - Adds multiple layers of entropy, decoy TLVs, and structural camouflage
    - Ensures stealth vs binwalk, hex editors, or signature scanners
    """
    ...
    def generate_integrity_anchor(blob: bytes) -> bytes:
        ...
        return b"\x13\x37ANCHOR" + anchor  # unique signature prefix for anti-tamper

    # Construct fake prelude layers
    pre_pad = entropy_noise(64)
    pre_tlv = build_fake_tlv(b"FAKE", len(capsule) // 2)
    pre_junk = build_fake_tlv(b"NULL", 32)

    # Real capsule (core IMG4 block)
    anchor = generate_integrity_anchor(capsule)
    core = anchor + capsule

    # Post obfuscation
    post_pad = entropy_noise(128)
    post_tlv = build_fake_tlv(b"GARB", 48)

    # Final obfuscated structure
    disguised = pre_pad + pre_tlv + pre_junk + core + post_tlv + post_pad
    total = align512(len(disguised))

    # Write into ELF buffer (masked memory map)
    buffer.seek(offset)
    buffer.write(disguised)

    # Ensure stealth block alignment
    buffer.write(b"\x00" * (total - len(disguised)))

    print(f"[♾️ IMG4 INJECTED]")

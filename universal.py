...
def generate_virtual_com_port():
    try:
        com_number = random.randint(10, 250)  # Avoid low COM ports
        com_id = f"COM{com_number}"
        print(f"[VCOM] Virtual COM Spoof -> {com_id}")
        return com_id
    except Exception as e:
        print(f"[ERROR] Failed to generate virtual COM: {e}")
        return "COM1337"  # Fallback ID

def generate_virtual_usb():
    try:
        # Random but safe VID and PID (avoid reserved ranges)
        spoof_vid = random.randint(0x1000, 0xFFFE)
        spoof_pid = random.randint(0x1000, 0xFFFE)

        # Random 12-character serial number
        serial_number = ''.join(random.choices('0123456789ABCDEF', k=12))

        # OUT Endpoints: Always include EP1
        out_eps = [1] + random.sample([ep for ep in range(2, 16)], k=random.randint(0, 2))

        # IN Endpoints: Choose 1–3 that don't overlap with OUT
        possible_in_eps = [ep for ep in range(1, 16) if ep not in out_eps]
        in_eps = random.sample(possible_in_eps, k=min(len(possible_in_eps), random.randint(1, 3)))

        print(f"[VUSB] VID:PID -> {spoof_vid:04X}:{spoof_pid:04X} | Serial -> {serial_number}")
        print(f"[VUSB] OUT Endpoints -> {out_eps}")
        print(f"[VUSB] IN Endpoints -> {in_eps}")
        ...
        class VirtualUSB:
            def __init__(self):
                ...
                self.out_endpoints = out_eps
                self.in_endpoints = in_eps

            def write(self, endpoint, data):
                if endpoint in self.out_endpoints:
                    print(f"[VUSB] EP{endpoint} <= {len(data)} bytes")
                else:
                    print(f"[VUSB] [WARN] Invalid OUT EP {endpoint}")

            def read(self, endpoint, size):
                if endpoint in self.in_endpoints:
                    dummy_data = os.urandom(size)
                    print(f"[VUSB] EP{endpoint} => {len(dummy_data)} bytes")
                    return dummy_data
                else:
                    print(f"[VUSB] [WARN] Invalid IN EP {endpoint}")
                    return None
        ...
        print(f"[ERROR] Failed to generate virtual USB device: {e}")
        return None
...
def serial_fallback(payload, port_override=None, hijack=False):
    ...
    # 👇 If no physical ports and hijack is requested (e.g., virtual mode), inject the expected VCOM
    if not ports and hijack:
        ports = ["/dev/ttyVCOMQSLCL"]

    for port in ports:
        if port == "/dev/ttyVCOMQSLCL":
            print(f"[VCOM] Virtual COM triggered — simulating QSLCL interaction on {port}")
            if not hijack:
                print("[VCOM] Hijack not enabled. Skipping simulation.")
                return
            try:
                print("[VCOM] Sending ELF payload...")
                time.sleep(0.2)
                print("[VCOM] Waiting for QSLCL raw response...")
                fake_responses = [
                    os.urandom(8),
                    b'\x51\x53\x4C\x43\x4C\x3A\x01\x00',  # Simulated header
                    struct.pack(">I", 0x00000000),        # Exec address
                    os.urandom(12),
                ]
                ...
                print(f"[ERROR] VCOM simulation error: {e}")
            return
        ...
        try:
            ...
            with serial.Serial(port, 115200, timeout=1) as ser:
                ...
                time.sleep(0.1)
                print("[SERIAL] Waiting for QSLCL response...")
                for _ in range(5):
                    resp = ser.readline()
                    if not resp:
                        break
                    print(f"[RESPONSE] {resp.decode(errors='ignore').strip()}")
                return
        except Exception as e:
...
def main():
    ...
    if virtual_usb:
        ...
        dev = generate_virtual_usb()
        if not dev:
            print("[FAIL] Virtual USB initialization failed.")
            sys.exit(1)

    elif virtual_com:
        spoofed_vcom = generate_virtual_com_port()
        print("[VCOM] Virtual COM mode triggered. qslcl.elf must handle this entry.")
        vcom_port = port_override if port_override else "/dev/ttyVCOMQSLCL"
        serial_fallback(payload, vcom_port, hijack=serial_hijack_mode)
        sys.exit(0)

...
def find_device():
    ...
    for pid in KNOWN_DFU_PIDS:
        ...
        if dev:
            print(f"[✓] Device found ➔ VID 0x{APPLE_VID:04X}, PID 0x{pid:04X}")
            return dev
    return None
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python universal.py qslcl.elf [--usb] [--virtual=usb] [--virtual=com] [--reboot] [--hijack] [--serial] [--port=/dev/ttyUSB0]")
    ...
    if virtual_usb:
        print("[🎭] Virtual USB mode triggered. QSLCL .elf must handle this entry.")
        class VirtualUSB:
        ...
        print("[🎭] Virtual COM mode triggered. QSLCL .elf must handle this entry.")
        serial_fallback(payload, port_override or "/dev/ttyVCOMQSLCL")
        sys.exit(0)
    else:
        dev = find_device()

...
# Partition Info (you can change based on your devices)
NUM_SECTORS = 8
SECTOR_SIZE = 512
PART_SIZE = NUM_SECTORS * SECTOR_SIZE 
START_ADDR = 0x5fa91000              
DUMP_DEST_OFFSET = 0x0          
...
def inject_full_fsg(ram, entropy):
    print(f"[🧬] Injecting full partition (0x0 → 0x{PART_SIZE:X}) with entropy...")
    for i in range(NUM_SECTORS):
        offset = START_ADDR + i * SECTOR_SIZE
        ram.seek(offset)
        ram.write(entropy_block(entropy, offset))
    print(f"[✔] Done writing partition into memory.")
...
def dump_fsg(ram, outfile, dump_offset):
    ...
    print(f"[📏] Real partitionsize: 0x{PART_SIZE:X} | Offset in file: 0x{dump_offset:X}")
...
def main():
    ...
    if check_fuse(ram):
        ...
        dump_fsg(ram, "devinfo.bin", dump_offset=DUMP_DEST_OFFSET)
    else:
        print("[🛑] FUSE check failed. Aborting partition dump.")
